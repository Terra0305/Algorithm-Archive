...
public class Baek1874 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        StringBuilder sb = new StringBuilder(); // 최종 출력용

        Stack<Integer> stack = new Stack<>();

        for (int i = 0; i < N; i++) { // N 만큼 반복 4가 입력되면 현재 count가 0이기 때문에, +를 4번해야함.
            int currentcount = Integer.parseInt(sc.nextLine()); // ex)) 5면
            ...
            if (!stack.isEmpty() && stack.peek() == currentcount) {
                stack.pop();
                sb.append("-\n");
            } else {
